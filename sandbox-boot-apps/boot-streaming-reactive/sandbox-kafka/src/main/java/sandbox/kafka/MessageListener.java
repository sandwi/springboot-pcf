...
public class MessageListener {
    ...
    public void listenGroupFoo(String message) {
        log.info("Received StockTicker in group 'foo': " + message);
        latch.countDown();
    }

    @KafkaListener(topics = "${message.topic.name}", groupId = "bar", containerFactory = "barKafkaListenerContainerFactory")
    public void listenGroupBar(String message) {
        log.info("Received StockTicker in group 'bar': " + message);
        latch.countDown();
    }

    @KafkaListener(topics = "${message.topic.name}", containerFactory = "headersKafkaListenerContainerFactory")
    public void listenWithHeaders(@Payload String message, @Header(KafkaHeaders.RECEIVED_PARTITION_ID) int partition) {
        log.info("Received StockTicker: " + message + " from partition: " + partition);
        latch.countDown();
    }
    ...
    public void listenWithFilter(String message) {
        log.info("Received StockTicker in filtered listener: " + message);
